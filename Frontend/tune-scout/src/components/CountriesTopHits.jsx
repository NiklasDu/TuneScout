  ...
  const handleSubmit = (e) => {
    ...
    const fetchTopTracks = async () => {
      const res = await fetch(
        `https://ws.audioscrobbler.com/2.0/?method=geo.gettoptracks&country=${selectedCountry}&limit=25&api_key=${apiKey}&format=json
`
      );
    ...
    <section className="bg-white dark:bg-gray-900 min-h-[75vh]">
      ...
      {!hasSearched && (
        <p className="text-center text-gray-500 text-lg py-8 dark:text-gray-400">
          Choose a country to see the top tracks of the country!
        </p>
      )}
      <div className="mx-auto max-w-screen-md px-8 flex gap-8">
        {hasSearched && (
          <>
              ...
              {topTracks.map((track, index) => (
                ...
                >
                  ...
                  <div className="flex flex-col justify-center w-4/5 pl-4 overflow-hidden">
                    <h1 className="mb-2 mt-2 text-2xl font-medium text-gray-800 truncate dark:text-white">
                      {track.name}
                    </h1>
                    <h2 className="text-lg text-gray-600 truncate dark:text-gray-300">
                      {track.artist.name}
                    </h2>
                  </div>
                </div>
